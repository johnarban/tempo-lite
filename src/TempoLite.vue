  ...
  > 
    <div class="content-with-sidebars">
      ...
      <div id="map-container">
        ...
        <div id="slider-row">
          <!-- <span style="font-size: 10px"> {{ maxIndex }}  - {{ minIndex }} &lt;?
          ({{ minSliderVals }} - 1)</span> -->
          <div v-if="maxIndex - minIndex + 1 < minSliderVals" id="time-chip-container">
            <!-- Accessiblity suggestion Generated by Copilot -->
            <label class="time-chip-label" for="time-chip-group">Select a Time</label>
            <v-chip-group
              id="time-chip-group"
              v-model="timeIndex"
              column
              ...
              >
              <v-chip
                v-for="i in rangeInclusive(minIndex,maxIndex)"
                :key="i"
                :value="i"
                @click="timeIndex = i"
                @keyup.enter="timeIndex = i"
                :color="i === timeIndex ? accentColor : 'white'"
                class="time-chip"
              >
                {{ chipTime(i) }}
              </v-chip>
...
export default defineComponent({
  ...
  methods: {
    ...
    rangeInclusive(n: number, max: number | undefined = undefined, step: number | undefined = undefined): number[] {
      if (max === undefined) {
        max = n;
        n = 0;
      }
      ...
      return Array.from({ length: Math.floor((max - n) / step + 1) }, (_, i) => n + i * step);
    },
    
    
    
    chipTime(index: number): string {
      const offset = getTimezoneOffset(this.selectedTimezone, this.date);
      const date = new Date(this.timestamps[index] + offset); 
      const hours = date.getUTCHours();
      const amPm = hours >= 12 ? "PM" : "AM";
      let hourValue = hours % 12;
      if (hourValue === 0) {
      ...
      // return `${this.date.getUTCMonth()+1}/${date.getUTCDate()}/${date.getUTCFullYear()} ${hourValue}:${date.getUTCMinutes().toString().padStart(2, '0')} ${amPm}`;
      return `${hourValue}:${date.getUTCMinutes().toString().padStart(2, '0')} ${amPm}`;
    },
